			...
			<?php if ( is_single() ) { ?>
				...
				<?php } ?>
                    ...
                    if ( comments_open() || get_comments_number() ) { ?>
                        <a href="<?php echo get_comments_link( get_the_ID() ); ?>" class="flex align-items-center bb-comments-wrap"><i class="bb-icon-comment"></i><span class="comments-count"><?php printf( _nx( '1 Comment', '%1$s Comments', get_comments_number(), 'comments title', 'buddyboss-theme' ), number_format_i18n( get_comments_number() ) ); ?></span></a>
				<?php } ?>
			<?php } ?>
		...
		if ( !is_single() && ( 'standard' === $blog_type || 'masonry' === $blog_type || 'grid' === $blog_type ) ) {
			...
			<?php if ( comments_open() || get_comments_number() ) { ?>
				<a href="<?php echo get_comments_link( get_the_ID() ); ?>" class="flex align-items-center bb-comments-wrap">
					...
					<span class="comments-count"><?php printf( _nx( '1 Comment', '%1$s Comments', get_comments_number(), 'comments title', 'buddyboss-theme' ), number_format_i18n( get_comments_number() ) ); ?></span>
				</a>
			<?php } ?>
