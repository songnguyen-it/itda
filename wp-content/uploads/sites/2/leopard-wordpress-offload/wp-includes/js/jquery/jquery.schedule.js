(function($){$.scheduler=function(){this.bucket={};return};$.scheduler.prototype={schedule:function(){var ctx={"id":null,"time":1000,"repeat":!1,"protect":!1,"obj":null,"func":function(){},"args":[]};function _isfn(fn){return(!!fn&&typeof fn!="string"&&typeof fn[0]=="undefined"&&RegExp("function","i").test(fn+""))};var i=0;var override=!1;if(typeof arguments[i]=="object"&&arguments.length>1){override=!0;i++}
...
ctx.protect=arguments[i++];if(typeof arguments[i]=="object"&&typeof arguments[i+1]=="string"&&_isfn(arguments[i][arguments[i+1]])){ctx.obj=arguments[i++];ctx.func=arguments[i++]}else if(typeof arguments[i]!="undefined"&&(_isfn(arguments[i])||typeof arguments[i]=="string"))
...
ctx[option]=arguments[0][option]}else{for(var option in arguments[0])
if(typeof ctx[option]!="undefined")
...
ctx.id=(String(ctx.repeat)+":"+String(ctx.protect)+":"+String(ctx.time)+":"+String(ctx.obj)+":"+String(ctx.func)+":"+String(ctx.args));if(ctx.protect)
...
ctx.func=ctx.obj[ctx.func];else ctx.func=eval("function () { "+ctx.func+" }")}
ctx._handle=this._schedule(ctx);this.bucket[ctx.id]=ctx;return ctx},reschedule:function(ctx){if(typeof ctx=="string")
ctx=this.bucket[ctx];ctx._handle=this._schedule(ctx);return ctx},_schedule:function(ctx){var trampoline=function(){var obj=(ctx.obj!=null?ctx.obj:ctx);(ctx.func).apply(obj,ctx.args);if(typeof(ctx._scheduler).bucket[ctx.id]!="undefined"&&ctx.repeat)(ctx._scheduler)._schedule(ctx);else delete(ctx._scheduler).bucket[ctx.id]};return setTimeout(trampoline,ctx.time)},cancel:function(ctx){if(typeof ctx=="string")
